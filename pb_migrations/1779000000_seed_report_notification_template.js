...
migrate((app) => {
  console.log("🚀 Seeding Professional Report Notification Template");

  const COLLECTION_NAME = "email_templates";
  const TEMPLATE_NAME = "Rapport Notificatie";

  try {
    const existing = app.findFirstRecordByFilter(COLLECTION_NAME, `name = '${TEMPLATE_NAME}'`);
    console.log(`   - Template '${TEMPLATE_NAME}' already exists. Skipping.`);
    return;
  } catch (_) {
    // Not found, continue to seed
  }
...
<body style="margin: 0; padding: 0; background-color: #f8fafc; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;">
  ...
  app.save(record);
  console.log(`   ✅ Seeded template: '${TEMPLATE_NAME}' with Unlayer design.`);

}, (app) => {
  try {
    const record = app.findFirstRecordByFilter("email_templates", "name = 'Rapport Notificatie'");
    app.delete(record);
  } catch (_) { }
});
