...
migrate((app) => {
    console.log("🚀 Starting migration: Ensure ploegen multi-press support (v3)");

    const ploegen = app.findCollectionByNameOrId("ploegen");

    // 1. Add 'presses' JSON field if missing
    ...
    records.forEach(record => {
        ...
        // Only migrate if presses is empty/null
        if (!currentPresses || (Array.isArray(currentPresses) && currentPresses.length === 0)) {
            ...
            if (persId) {
                try {
                    ...
                    if (pressName) {
                        ...
                        app.save(record);
                        migrated++;
                        console.log(`   + Migrated ploeg "${record.get("naam")}": pers "${persId}" → presses ["${pressName}"]`);
                    }
                } catch (e) {
            ...
            console.log(`   - Ploeg "${record.get("naam")}" already has presses: ${JSON.stringify(currentPresses)}, skipping.`);
        }
    });

    // 4. Set active=true for records where not set
    records.forEach(record => {
