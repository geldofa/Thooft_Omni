...
export function CategoryManagement() {
    ...
    const [formData, setFormData] = useState({
        name: '',
        subtexts: {} as Record<string, string>,
        pressIds: [] as string[],
        active: true
    });
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="grid gap-4 py-4">
                            ...
                            <div className="grid gap-3">
                                <Label>Beschikbare Persen & Subtexts</Label>
                                <div className="space-y-3 border rounded-md p-3">
                                    {activePressRecords.map((press) => {
                                        const isSelected = formData.pressIds.includes(press.id);
                                        return (
                                            <div key={press.id} className="flex flex-col space-y-2 pb-2 border-b last:border-0 last:pb-0">
                                                ...
                                                {isSelected && (
                                                    <div className="pl-6">
                                                        <Input
                                                            ...
                                                            placeholder={`Subtext voor ${press.name}`}
                                                            value={formData.subtexts[press.id] || ''}
                                                            onChange={(e) => setFormData(prev => ({
                                                                ...prev,
                                                                subtexts: {
                                                                    ...prev.subtexts,
                                                                    [press.id]: e.target.value
                                                                }
                                                            }))}
                                                        />
                                                    </div>
