...
export function OperatorManagement({ operators, ploegen, presses, isLoading, onRefresh }: OperatorManagementProps) {
  ...
  const [selectedPressesForPloeg, setSelectedPressesForPloeg] = useState<string[]>([]);
  ...
  useEffect(() => {
    if (isPloegDialogOpen && !editingPloeg) {
      setPloegFormData(prev => ({ ...prev, operatorIds: [] }));
    }
  }, [selectedPressesForPloeg]);

  useEffect(() => {
    const isAutoGenerated = operators.some(op => `Team ${op.name}` === ploegFormData.name);
    ...
    if (ploegFormData.operatorIds.length > 0) {
      if (ploegFormData.name.trim() === '' || isAutoGenerated) {
        ...
        if (operator) {
          setPloegFormData(prev => ({ ...prev, name: `Team ${operator.name}` }));
        }
      }
  ...
  const handleDeletePloeg = async (id: string, name: string) => {
    try {
      ...
      onRefresh();
      toast.success(`Team "${name}" verwijderd`);
      ...
      addActivityLog({
        ...
        entityId: id,
        entityName: name,
        details: `Deleted team: ${name}`,
        oldValue: `Naam: ${name}`
      });
    } catch (err) {
      console.error("Failed to delete ploeg:", err);
      toast.error('Verwijderen team mislukt');
    }
  };
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        description="Beheer operators, teams en externe entiteiten"
        icon={Users}
        className="mb-2"
      />
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="tab-pill-list">
          ...
          <TabsTrigger value="ploegen" className="tab-pill-trigger">Teams</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="ploegen" className="space-y-4">
          ...
          <div className="flex items-center justify-between mt-4">
            ...
            <Button onClick={() => { setEditingPloeg(null); setSelectedPressesForPloeg([]); setPloegFormData({ name: '', operatorIds: [], active: true }); setIsPloegDialogOpen(true); }}>
                          ...
                          <div className="flex justify-end gap-2">
                            <Button
                              ...
                              onClick={() => {
                                ...
                                setSelectedPressesForPloeg([...ploeg.presses]);
                                setPloegFormData({
                                  name: ploeg.name,
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    Weet u zeker dat u "{ploeg.name}" wilt verwijderen? Deze actie kan niet ongedaan worden gemaakt.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel>Annuleren</AlertDialogCancel>
      ...
      <Dialog open={isPloegDialogOpen} onOpenChange={(open) => {
        ...
        if (!open) {
          ...
          setSelectedPressesForPloeg([]);
        }
      }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editingPloeg ? 'Team Bewerken' : 'Nieuw Team Toevoegen'}</DialogTitle>
            <DialogDescription>
              Maak een team van 2-3 operatoren die samenwerken.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={async (e) => {
            e.preventDefault();
            if (selectedPressesForPloeg.length > 0) {
              const finalPloegData = { ...ploegFormData, presses: selectedPressesForPloeg };
              try {
                if (editingPloeg) {
                  ...
                  toast.success('Team succesvol bijgewerkt');
                } else {
                ...
                toast.error('Opslaan team mislukt');
              }
            } else {
              toast.error("Selecteer minstens één pers");
            }
          }}>
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                <Label htmlFor="ploeg-name">Team Naam *</Label>
                <Input
                  id="ploeg-name"
                ...
                <div className="flex flex-wrap gap-2 border rounded-md p-3">
                  {activePresses.map(press => (
                    <Button
                      key={press.id}
                      type="button"
                      variant={selectedPressesForPloeg.includes(press.name) ? 'default' : 'outline'}
                      size="sm"
                      onClick={() => {
                        setSelectedPressesForPloeg(prev =>
                          prev.includes(press.name)
                            ? prev.filter(p => p !== press.name)
                            : [...prev, press.name]
                        );
                      }}
                    >
                  ...
                  {[0, 1, 2].map((index) => (
                    <div key={index} className="flex items-center gap-2">
                      ...
                      >
                        ...
                        {operators
                          .filter(op => op.active && (selectedPressesForPloeg.length === 0 || selectedPressesForPloeg.some(sp => op.presses.includes(sp))) && (!ploegFormData.operatorIds.includes(op.id) || ploegFormData.operatorIds[index] === op.id))
                          .map(op => (
            ...
            <DialogFooter>
              ...
              <Button type="submit">
                {editingPloeg ? 'Team Bijwerken' : 'Team Toevoegen'}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
